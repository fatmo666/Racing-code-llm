...
EXP_ST u8 skip_deterministic, /* Skip deterministic stages?       */
    ...
    crashes_bitmap_changed = 1, non_crashes_bitmap_changed = 1,
...
EXP_ST u8 virgin_bits[MAP_SIZE], /* Regions yet untouched by fuzzing */
    ...
    virgin_bits_crashes[INST_SIZE*96+BB_SIZE*32], virgin_bits_non_crashes[INST_SIZE*96+BB_SIZE*32],
...
static FILE *ranked_file; /* rank file              */
...
struct queue_entry {
  ...
  u32 bitmap_size,             /* Number of bits set in bitmap     */
      value_cksum, exec_cksum; /* Checksum of the execution trace  */
...
static struct queue_entry *queue,        /* Fuzzing queue (linked list)      */
    *queue_cur,                          /* Current offset within the queue  */
        *queue_first = NULL, *queue_top, /* Top of the list                  */
...
double predicate_score[INST_SIZE+BB_SIZE];    // score of predicate
u64 predicate_boundary[INST_SIZE+BB_SIZE];    // boundary of predicate
enum Predicate predicate_type[INST_SIZE+BB_SIZE];  // type of predicate
double predicate_order[INST_SIZE+BB_SIZE];

u32 CoP_cnt = 0;    // counter for predicates with counterexample
double CoP_rate = 0;
u32 target_predicate[TOP_K];   // id of target predicate

double cumulative_predicate_reward[INST_SIZE+BB_SIZE];
u32 predicate_selected_cnt[INST_SIZE+BB_SIZE];
...
double avg_predicate_rewards[TOP_K];
double d_predicate[TOP_K], d_predicate_prime[TOP_K];
...
static void select_predicate(void){
  ...
  memset(d_predicate, 0, sizeof(double)*TOP_K);
  ...
  for (u32 k = 0; k < TOP_K; k++) {
    ...
    if(all_reward>0)
      ...
      d_predicate[k]=1.000/TOP_K;
  }
}

static u32 sample_predicate(void){
  for(u32 k=0; k<TOP_K; k++)
    d_predicate_prime[k] = (1-predicate_selection_gamma)*d_predicate[k]+predicate_selection_gamma/TOP_K;
...
static void update_predicate_state(double reward){
  ...
  for(u32 i=0;i<INST_SIZE+BB_SIZE;i++){
    if (!queue_cur->trace_bits->is_visited[i])
      continue;
...
static void update_predicate_selection_gamma(void) {
  ...
  if(selected_predicate!=0xffffffff && predicate_selected_cnt[selected_predicate]!=backup_predicate_selected_cnt){
    ...
    predicate_selection_gamma = TOP_K*1.000/(2*TOP_K+predicate_selection_adaptive);
  }
  return;
}
...
static u32 sample_loc(s32 len){
  ...
  if(target_loc_cnt<len){
    double p_target_loc = 1-loc_selection_gamma+loc_selection_gamma*target_loc_cnt/len;
    if(random_double>=p_target_loc){
      posn = target_loc_cnt + (int)((random_double-p_target_loc)/(loc_selection_gamma/len));
      p_selected_loc+=loc_selection_gamma/len;
  ...
  for(u32 p=0; p<target_loc_cnt; p++)
    d_loc_prime[p] = (1-loc_selection_gamma)*d_loc[p]+loc_selection_gamma/len;
...
struct value_info {
  u64 value;
  u32 crashes_count;
  u32 non_crashes_count;
  struct queue_entry *crashes_top_rated;
  u8 crashes_was_fuzzed;
  struct queue_entry *non_crashes_top_rated;
  u8 non_crashes_was_fuzzed;
...
struct inst_info {
  u32 crash_count;
  u32 non_crash_count;
  struct value_info *min_value;
  struct value_info *max_value;
  struct value_info *carry_flag;
  struct value_info *parity_flag;
  struct value_info *adjust_flag;
  struct value_info *zero_flag;
  struct value_info *sign_flag;
  struct value_info *trap_flag;
  struct value_info *interrupt_flag;
  struct value_info *direction_flag;
...
struct control_flow_info global_bb_info[BB_SIZE];

struct bb_source_info{
    char BBstartSourceCodeInfo[50];
    char BBendSourceCodeInfo[50];
};
struct bb_source_info global_bb_source_info[BB_SIZE];
// -------------------------------------------------------
...
static void update_sorted_value_info(struct value_info **list, u64 value, u8 label) {
  ...
  if (current && current->value == value) {
    if (label)
      current->crashes_count++;
    else
      current->non_crashes_count++;
  } else {
    struct value_info *new_value_info = (struct value_info *)malloc(sizeof(struct value_info));
    new_value_info->value = value;
    new_value_info->crashes_count = label ? 1 : 0;
    new_value_info->non_crashes_count = label ? 0 : 1;
    new_value_info->crashes_top_rated = 0;
    new_value_info->crashes_was_fuzzed = 0;
    new_value_info->non_crashes_top_rated = 0;
    new_value_info->non_crashes_was_fuzzed = 0;

    new_value_info->next = current;
    if (prev) {
...
static void add_to_global_values(u8 label) {
  if (label) {
    if(global_crash_count==0){
      for (u32 j = 0; j < INST_SIZE+BB_SIZE; j++)
        predicate_order[j] = trace_bits->exec_order[j];
      global_last_id = trace_bits->last_inst_id;
    }
    global_crash_count++;
  } else
    global_non_crash_count++;
  
  if(trace_bits->stack_start && trace_bits->stack_start <stack_start)
    stack_start = trace_bits->stack_start;

  if(trace_bits->stack_end>stack_end)
    stack_end = trace_bits->stack_end;

  if(trace_bits->heap_start && trace_bits->heap_start <heap_start)
    heap_start = trace_bits->heap_start;

  if(trace_bits->heap_end>heap_end)
    heap_end = trace_bits->heap_end;
  ...
  for (u32 i = 1; i < BB_SIZE; i++) {
    ...
    for(u32 j=start_index; j<start_index+cnt_succ; j++){
...
int is_heap(u64 v) {
  if((v & 0xffff600000000000) == 0x600000000000) //ASAN
    return 1;
...
double compute_reward(){
  struct rank_info * last_rank = global_rank_info;
  if(last_rank==NULL)
    return 0;
  struct rank_info * pre_rank = global_rank_info->next;
  if(pre_rank==NULL)
    return 0;

  u32 last_rank_vec[INST_SIZE+BB_SIZE];
  u32 pre_rank_vec[INST_SIZE+BB_SIZE];
  u32 union_rank_ids[TOP_K*2];
  u32 union_cnt = 0;

  memset(last_rank_vec, 0xff, sizeof(u32)*(INST_SIZE+BB_SIZE));
  memset(pre_rank_vec, 0xff, sizeof(u32)*(INST_SIZE+BB_SIZE));
  memset(union_rank_ids, 0xff, sizeof(u32)*TOP_K*2);
  ...
  u32 inversion_cnt = 0,all_cnt=0;
  for(u32 i=0;i<union_cnt;i++){
    for(u32 j=i+1;j<union_cnt;j++){
      if(last_rank_vec[union_rank_ids[i]]==last_rank_vec[union_rank_ids[j]])
        continue;
      if(pre_rank_vec[union_rank_ids[i]]==pre_rank_vec[union_rank_ids[j]])
        continue;
      
      all_cnt++;
      u8 last_cmp=last_rank_vec[union_rank_ids[i]]<last_rank_vec[union_rank_ids[j]]? '<' : '>';
      u8 pre_cmp=pre_rank_vec[union_rank_ids[i]]<pre_rank_vec[union_rank_ids[j]]? '<': '>';
  ...
  if(all_cnt)
    global_rank_info->rank_distance = inversion_cnt*1.00/all_cnt;

  fprintf(ranked_file, "global_rank_info->rank_distance: %f\n", global_rank_info->rank_distance);
  fprintf(ranked_file, "CoP_rate: %f\n", CoP_rate);
  fflush(ranked_file);

  double reward = global_rank_info->rank_distance+CoP_rate;
  return reward;
}
...
int converge(){
  u8 converged = 1;
  if(!global_rank_info || global_rank_info->inputs_count<12 || predicate_score[target_predicate[0]]==predicate_score[target_predicate[49]])
    converged = 0;
  struct rank_info * tmp_rank = global_rank_info;
  while(tmp_rank && (global_rank_info->inputs_count - tmp_rank->inputs_count)<10){
    if(tmp_rank->cycle<2)
      converged = 0;
    
    if(tmp_rank->rank_distance>0.01)
      converged = 0;
...
double calc_MutualInformation(double cpi1, double cfi1, double cpi2, double cfi2){
  ...
  double I_y_b_global = H_y_global + ci1/N_global*(H11+H12) + ci2/N_global*(H21+H22);
  return I_y_b_global;
}
...
static void update_predicates() {
  ...
  while (i < INST_SIZE) {
    ...
    int has_NonSHValue = 0;

    struct value_info *temp_min = global_inst_info[i].min_value;
    while (temp_min) {
      if(temp_min->crashes_count && !is_stack(temp_min->value) && !is_heap(temp_min->value)) {
        has_NonSHValue = 1;
        break;
      }
    ...
    if (has_NonSHValue || i==global_last_id)
    ...
    while (temp_max) {
      if(temp_max->crashes_count && !is_stack(temp_max->value) && !is_heap(temp_max->value)) {
        has_NonSHValue = 1;
        break;
      }
    ...
    while (temp_flag)
    {
      if(temp_flag->value){
        ...
        if (score > max_score && t_crash>=global_crash_count - t_crash) {
          max_score = score;
          max_boundary = 1;
          max_predicate = INTERRUPT_FLAG_SET;
        }
        break;
      }
      temp_flag = temp_flag->next;
    ...
    while (temp_flag)
    {
      if(temp_flag->value){
        ...
        if (score > max_score && t_crash>=global_crash_count - t_crash) {
          max_score = score;
          max_boundary = 1;
          max_predicate = DIRECTION_FLAG_SET;
        }
        break;
      }
      temp_flag = temp_flag->next;
    ...
    while (temp_flag)
    {
      if(temp_flag->value){
        ...
        if (score > max_score && t_crash>=global_crash_count - t_crash) {
          max_score = score;
          max_boundary = 1;
          max_predicate = OVERFLOW_FLAG_SET;
        }
        break;
      }
      temp_flag = temp_flag->next;
...
const char* predicateToString(enum Predicate pred) {
  switch (pred) {
    case MIN_VAL_LEQ:
      return "min_val_leq";
    case MAX_VAL_GT:
      return "max_val_gt";
    case CARRY_FLAG_SET:
      return "carry_flag_set";
    case PARITY_FLAG_SET:
      return "parity_flag_set";
    case ADJUST_FLAG_SET:
      return "adjust_flag_set";
    case ZERO_FLAG_SET:
      return "zero_flag_set";
    case SIGN_FLAG_SET:
      return "sign_flag_set";
    case TRAP_FLAG_SET:
      return "trap_flag_set";
    case INTERRUPT_FLAG_SET:
      return "interrupt_flag_set";
    case DIRECTION_FLAG_SET:
      return "direction_flag_set";
    case OVERFLOW_FLAG_SET:
      return "overflow_flag_set";
    case HAS_EDGE_TO:
      return "has_edge_to";
    case NUM_SUCCESSORS_GT:
...
static void rank(){
  memset(target_predicate, 0xff, sizeof(u32) * TOP_K);

  for (u32 i = 0; i < INST_SIZE + BB_SIZE; ++i) {
    if (predicate_score[i] == 0)
      continue;
    
    if (target_predicate[TOP_K-1] != 0xffffffff && predicate_score[i]<predicate_score[target_predicate[TOP_K-1]])
      continue;
    ...
    for (u32 j = 0; j < TOP_K; j++) {
      ...
      if (predicate_score[i] > predicate_score[target_predicate[j]]) {
        memcpy(target_predicate + j + 1, target_predicate + j, (TOP_K - j - 1)*sizeof(u32));
        target_predicate[j] = i;
        break;
      } else if (predicate_score[i] == predicate_score[target_predicate[j]]) {
        if (predicate_order[i] < predicate_order[target_predicate[j]]) {
          memcpy(target_predicate + j + 1, target_predicate + j, (TOP_K - j - 1)*sizeof(u32));
          target_predicate[j] = i;
          break;
        }
  ...
  temp_rank->cycle = queue_cycle;
  temp_rank->rank_ids = (u32 *)malloc(sizeof(u32)*TOP_K);
  memset(temp_rank->rank_ids, 0xff, sizeof(u32)*TOP_K);
  ...
  for (u32 j = 0; j<TOP_K && target_predicate[j]!=0xffffffff; j++) {
    ...
    else if(predicate_type[id]==HAS_EDGE_TO)
    ...
    else if(predicate_type[id]==NUM_SUCCESSORS_GT)
  ...
  global_rank_info = temp_rank;
}

static void root_cause_analysis(){
  update_predicates();
  rank();
...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det, u8 label) {
  ...
  q->trace_bits = (struct exec_info *)ck_alloc(sizeof(struct exec_info));
  ...
  /* Set next_100 pointer for every 100th element (index 0, 100, etc) to allow
   * faster iteration. */
...
static inline u8 has_new_bits(u8 *virgin_map) {
  ...
  u64 *current = (u64 *)trace_bits->coverage;
  ...
  u32 *current = (u32 *)trace_bits->coverage;
...
static inline u8 has_new_crashes_values() {
  ...
  for(i=0; i<INST_SIZE; i++){
    if (trace_bits->is_visited[i]== 0){
      if(global_inst_info[i].crash_count==global_crash_count){
        if(is_target_inst(i)){
          CoP_cnt++;
        }
      }
    ...
    }else if (predicate_type[i]==CARRY_FLAG_SET || predicate_type[i]==PARITY_FLAG_SET || predicate_type[i]==ADJUST_FLAG_SET || predicate_type[i]==ZERO_FLAG_SET || predicate_type[i]==SIGN_FLAG_SET || predicate_type[i]==TRAP_FLAG_SET || predicate_type[i]==INTERRUPT_FLAG_SET || predicate_type[i]==DIRECTION_FLAG_SET || predicate_type[i]==OVERFLOW_FLAG_SET){
      u64 eflags = trace_bits->eflag[i];
  ...
  while(i--){
    if (trace_bits->is_visited[INST_SIZE+i]== 0){
      if(global_bb_info[i].crash_count==global_crash_count){
    ...
    if (predicate_type[INST_SIZE+i]==HAS_EDGE_TO || predicate_type[INST_SIZE+i]==NUM_SUCCESSORS_GT) {
      u32 start_index = trace_bits->bb_start_index[i];
      u16 cnt_succ = trace_bits->bb_succ_cnt[i];
      ...
      if((predicate_type[INST_SIZE+i]==HAS_EDGE_TO && found==0) || (predicate_type[INST_SIZE+i]==NUM_SUCCESSORS_GT && exec_cnt_succ<=predicate_boundary[INST_SIZE+i])){
...
static inline u8 has_new_non_crashes_values() {
  ...
  while(i--){
    ...
    if(global_bb_info[i].non_crash_count==0 && predicate_type[INST_SIZE+i]==HAS_EDGE_TO){
      if(CoP_cnt==0){
        CoP_cnt++;
      }
      continue;
    ...
    if (predicate_type[INST_SIZE+i]==HAS_EDGE_TO || predicate_type[INST_SIZE+i]==NUM_SUCCESSORS_GT) {
      ...
      if((predicate_type[INST_SIZE+i]==HAS_EDGE_TO && found==1) || (predicate_type[INST_SIZE+i]==NUM_SUCCESSORS_GT && exec_cnt_succ>predicate_boundary[INST_SIZE+i])){
        u32 edges_cksum = hash32(&bb_id_sum, 8, HASH_CONST);
        edges_cksum = edges_cksum & 0xff; 
...
static void update_value_top_rated(struct value_info *value_info, struct queue_entry *q, u64 fav_factor, u8 is_crash) {
  if(is_crash){
    if (!value_info->crashes_top_rated || fav_factor <= value_info->crashes_top_rated->exec_us * value_info->crashes_top_rated->len) {
      ...
      value_info->crashes_was_fuzzed = 0;
    }
  }else{
    if (!value_info->non_crashes_top_rated || fav_factor <= value_info->non_crashes_top_rated->exec_us * value_info->non_crashes_top_rated->len) {
      value_info->non_crashes_top_rated = q;
      value_info->non_crashes_was_fuzzed = 0;
    }
  }
}

static void update_value_bitmap_score(struct queue_entry *q) {
  ...
  for (i = 0; i < INST_SIZE; i++) {
    ...
    if ((trace_bits->is_visited[i] & 2) == 0)
      continue;
  ...
  for (u32 k = 0; k < TOP_K; k++) {
    ...
    if(predicate_type[i]==MIN_VAL_LEQ){
      ...
      if (unlikely(temp_v)){
        ...
        temp_v->crashes_was_fuzzed = 1;
        queue_first->favored = 1;
        if(global_bb_info[i-INST_SIZE].non_crash_count==0)
          queue_first->was_fuzzed=0;
        selected_predicate=i;
        break;
      }
...
EXP_ST void setup_shm(void) {
  ...
  memset(virgin_bits_crashes, 0, INST_SIZE*96+BB_SIZE*32);
  memset(virgin_bits_non_crashes, 0, INST_SIZE*96+BB_SIZE*32);

  memset(global_inst_info,0,sizeof(struct inst_info)*INST_SIZE);
  memset(global_bb_info,0,sizeof(struct control_flow_info)*BB_SIZE);
  memset(global_inst_source_info, 0, sizeof(struct inst_source_info)*INST_SIZE);
  memset(global_bb_source_info, 0, sizeof(struct bb_source_info)*BB_SIZE);

  memset(predicate_score, 0, sizeof(double) * (INST_SIZE+BB_SIZE));
  memset(predicate_boundary, 0, sizeof(u64) * (INST_SIZE+BB_SIZE));
  memset(predicate_type, NULL_PREDICATE, sizeof(enum Predicate) * (INST_SIZE+BB_SIZE));
  memset(predicate_order, 0, sizeof(double) * (INST_SIZE+BB_SIZE));
  memset(target_predicate, 0xff, sizeof(u32) * TOP_K);

  memset(cumulative_predicate_reward, 0, sizeof(double)*(INST_SIZE+BB_SIZE));
  memset(predicate_selected_cnt, 0, sizeof(u32)*(INST_SIZE+BB_SIZE));
  ...
  trace_bits = (struct exec_info*)shmat(shm_id, NULL, 0);
...
static void read_testcases(void) {
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    add_to_queue(fn, st.st_size, passed_det, crash_mode ? 1 : 0);
  }
...
EXP_ST void init_forkserver(char **argv) {
  ...
  if (!forksrv_pid) {
    ...
    *(u32 *)trace_bits->coverage = EXEC_FAIL_SIG;
    exit(0);
  }
  ...
  if (*(u32 *)trace_bits->coverage == EXEC_FAIL_SIG)
...
static u8 run_target(char **argv, u32 timeout) {
  ...
  memset((u8*)trace_bits, 0, sizeof(struct exec_info));
  ...
  tb4 = *(u32 *)trace_bits->coverage;

#ifdef WORD_SIZE_64
  classify_counts((u64 *)trace_bits->coverage);
#else
  classify_counts((u32 *)trace_bits->coverage);
#endif /* ^WORD_SIZE_64 */
...
static u8 calibrate_case(char **argv, struct queue_entry *q, u8 *use_mem,
  ...
  static struct exec_info first_trace;
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (stop_soon || (fault != crash_mode && fault != FAULT_NONE))
      goto abort_calibration;

    if (!dumb_mode && !stage_cur && !count_bytes(trace_bits->coverage)) {
      fault = FAULT_NOINST;
      goto abort_calibration;
    }

    cksum = hash32(trace_bits->coverage, MAP_SIZE, HASH_CONST);
    ...
    if (q->exec_cksum != cksum) {
      ...
      if (q->exec_cksum) {
        ...
        for (i = 0; i < MAP_SIZE; i++) {
          ...
          if (!var_bytes[i] && first_trace.coverage[i] != trace_bits->coverage[i]) {
  ...
  q->bitmap_size = count_bytes(trace_bits->coverage);
  q->handicap = handicap;
  q->cal_failed = 0;
...
static void check_map_coverage(void) {
  ...
  if (count_bytes(trace_bits->coverage) < 100)
    return;

  for (i = (1 << (MAP_SIZE_POW2 - 1)); i < MAP_SIZE; i++)
    if (trace_bits->coverage[i])
      return;
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    fn = alloc_printf("%s/queue_crashes/id:%06u,%s", out_dir, queued_paths,
                      describe_op(hnb));

#else

    fn = alloc_printf("%s/queue_crashes/id_%06u", out_dir, queued_paths);

#endif /* ^!SIMPLE_FILES */

    
    add_to_queue(fn, len, 0, 1);

    queue_top->has_new_cov = 1;
    queued_with_cov++;
    queue_top->exec_cksum = hash32(trace_bits->coverage, MAP_SIZE, HASH_CONST);
    queue_top->value_cksum =
        hash32(trace_bits->min_value,INST_SIZE*16, HASH_CONST);

    /* Try to calibrate inline;*/
    ...
    fn = alloc_printf("%s/queue_non_crashes/id:%06u,%s", out_dir, queued_paths,
                      describe_op(hnb));

#else

    fn = alloc_printf("%s/queue_non_crashes/id_%06u", out_dir, queued_paths);
...
#ifdef WORD_SIZE_64
      simplify_trace((u64 *)trace_bits->coverage);
#else
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s/queue_crashes", out_dir);
  if (delete_files(fn, CASE_PREFIX))
    goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/queue_non_crashes", out_dir);
  if (delete_files(fn, CASE_PREFIX))
    goto dir_cleanup_failed;
  ck_free(fn);
  ...
  fn = alloc_printf("%s/ranked_file", out_dir);
  if (unlink(fn) && errno != ENOENT)
    goto dir_cleanup_failed;
  ck_free(fn);
...
static u8 trim_case(char **argv, struct queue_entry *q, u8 *in_buf) {
  ...
  static u8 tmp[64];
  static struct exec_info clean_trace;
  ...
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, TRIM_MIN_BYTES)) {
    ...
    while (remove_pos < q->len) {
      ...
      cksum = hash32(trace_bits->coverage, MAP_SIZE, HASH_CONST);
      value_cksum = hash32(trace_bits->min_value, INST_SIZE*16, HASH_CONST);
      ...
      if (cksum == q->exec_cksum && value_cksum == q->value_cksum) {
        ...
        if (!needs_write) {
          ...
          needs_write = 1;
          memcpy(&clean_trace, trace_bits, sizeof(struct exec_info));
        }
  ...
  if (needs_write) {
    ...
    memcpy(trace_bits, &clean_trace, sizeof(struct exec_info));
    update_value_bitmap_score(q);
  }
...
static u8 fuzz_one(char **argv) {
  ...
  if (pending_favored) {
    ...
    /* If we have any favored, non-fuzzed new arrivals in the queue,
       ...
       cases. */
    if ((!queue_cur->favored) && UR(100) < SKIP_TO_NEW_PROB)
      return 1;

    if (queue_cur->favored && queue_cur->was_fuzzed && UR(100) < SKIP_OLD_FAV_PROB)
      return 1;
  ...
  if (queue_cur->cal_failed) {
    ...
    if (queue_cur->cal_failed < CAL_CHANCES) {
      ...
      update_value_bitmap_score(queue_cur);
    }
...
#define FLIP_BIT(_ar, _b)                                                      \
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    if (!dumb_mode && (stage_cur & 7) == 7) {
      ...
      u32 cksum = hash32(trace_bits->coverage, MAP_SIZE, HASH_CONST);
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      u8 op = UR_op(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0));
      switch (op) {
      case 0:
      ...
      case 1: 
        ...
        case 11 ... 12: {
            ...
            del_from = UR_loc(temp_len - del_len + 1);
            posn = del_from;
        ...
        case 14: {
            ...
            copy_to   = UR_loc(temp_len - copy_len + 1);
            posn = copy_to;
        ...
        case 15: {
            ...
            if (!extras_cnt || (a_extras_cnt && UR(2))) {
              ...
              insert_at = UR_loc(temp_len - extra_len + 1);
              posn = insert_at;
        ...
        case 16: {
            ...
            u32 use_extra, extra_len, insert_at = UR_loc(temp_len + 1);
            u8* new_buf;
            posn = insert_at;
    ...
    if(global_rank_info && ((get_cur_time() - start_time) / 1000-global_rank_info->update_time>1800)){
      stop_soon = 2;
      goto abandon_entry;
    }
    ...
    if (queued_paths != havoc_queued) {
      ...
      if (perf_score < HAVOC_MAX_MULT * 100/2) {
        stage_max  *= 2;
        perf_score *= 2;
      }
...
static void usage(u8 *argv0) {
  ...
  SAYF(
      ...
      "  -s            - file for storing source code information\n"
...
EXP_ST void setup_dirs_fds(void) {
  ...
  tmp = alloc_printf("%s/queue_non_crashes", out_dir);
  if (mkdir(tmp, 0700))
    PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  tmp = alloc_printf("%s/queue_crashes", out_dir);
  if (mkdir(tmp, 0700))
    PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
  ...
  tmp = alloc_printf("%s/ranked_file", out_dir);
  fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, 0600);
  if (fd < 0)
    PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  ranked_file = fdopen(fd, "w");
  if (!ranked_file)
    PFATAL("fdopen() failed");
}
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:s:S:M:x:QV")) > 0)
    ...
    case 's': /* file for storing source code information*/

      if (sourceInfo_file)
        FATAL("Multiple -s options not supported");
      sourceInfo_file = optarg;
      break;
...
stop_fuzzing:
  ...
  fprintf(ranked_file, "Finished at: %s!\n", DTD(get_cur_time(), start_time));
  fflush(ranked_file);
