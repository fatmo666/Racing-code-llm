...
BASE_DIR="/Racing-eval/examples/21-ezXML-zero_day"; \
...
\
# 使用 jq 的 --rawfile 参数直接读取文件，然后通过管道将构建好的 JSON 发送给 curl
jq -n \
...
\
jq -r '.candidates[0].content.parts[0].text' "$GEMINI_RAW_OUTPUT" | sed 's/^```json//; s/```$//' | jq '.' > "$FINAL_REPORT_FILE" && \
\
# Final Check
if [ $? -eq 0 ] && [ -s "$FINAL_REPORT_FILE" ]; then
    echo "[+] AI analysis successful. Final report saved to $FINAL_REPORT_FILE"
    # rm "$GEMINI_RAW_OUTPUT" # 可选：如果成功，删除原始输出文件
else
    echo "[!] ERROR: AI analysis step failed. Check the raw output for error messages:"
    cat "$GEMINI_RAW_OUTPUT"
    exit 1
fi